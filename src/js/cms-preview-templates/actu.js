...
export default class ActuPreview extends React.Component {
  render() {
    ...
    const illu = getAsset(entry.getIn(["data", "illu"]));
    ...
    if (logo) {
        logoDiv = (
            <div class="w-24 h-24">
                <a href={ entry.getIn(["data", "link"]) } target="_blank" class="w-24 h-24">
                    <img src={ logo } class="object-contain rounded-full border-rebeige border-4 w-24 h-24" />
                </a>
            </div>
        );
    }
        ...
        <div> 
            <div className="w-11/12 lg:w-4/6 m-auto pt-8 pb-4 flex flex-row px-2 justify-center tracking-tighter">
                <div className="flex flex-col md:flex-row w-full items-center">
                    <div className="w-full max-w-xs md:max-w-sm h-full rounded-lg">{ illu && <img className="rounded-lg object-contain object-top w-full" src={ illu } /> }</div>
